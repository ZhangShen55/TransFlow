...
同日补充 19 节课程低前缀缓存测试：19 个文件共 2,964 条文本，补齐至 3,840 条后切成 64 个互不相同的 60 条请求，6 种目标语言共 23,040 个翻译单元。64 并发一次性处理耗时 98.390 秒，64/64 HTTP 200，前缀缓存命中率 32.097%，KV Cache 峰值 13.612%，无抢占或 OOM。按本轮工作量折算，4090D 约 419 节 335 段课程等效量/小时；据此估算独占 3090 约 189-252 节、4070 Ti Super 约 210-273 节/小时，均需目标硬件复测。详细结果见 `docs/benchmark-results.md` 和 `docs/reports/course-19-real-c64-r64.json`。
